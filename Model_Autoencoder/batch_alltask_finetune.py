...
print(hsp_str)
print(pct_str)
output_folder = '/users/nivl/data/autoencoder/classifier/{}/{}{}/{}{}{}_hsp{}_pct{}_{}_batch'.format(mode, month, day, hour, minute, sec, hsp_str, pct_str, pretrained_model)
...
for task_id, task in enumerate(task_lst):
    ...
    for sbj in hcp_sbj:
        ...
        if train_samples is None:
            train_samples = torch.from_numpy(sbj_samples)
            train_labels = sbj_labels
        else:
            train_samples = torch.cat((train_samples, torch.from_numpy(sbj_samples)))
    ...
    for sbj in retest_sbj:
        ...
        if test_samples is None:
            test_samples = torch.from_numpy(sbj_samples)
            test_labels = sbj_labels
        else:
            test_samples = torch.cat((test_samples, torch.from_numpy(sbj_samples)))
    ...
    test_batch = min(samples_per_sbj)
    ...
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=test_batch, shuffle=False)
