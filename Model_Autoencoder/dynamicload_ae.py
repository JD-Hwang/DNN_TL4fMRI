...
output_folder = '/users/nivl/data/autoencoder/hsp/{}{}/hsp{}_{}pct'.format(day, month, hsp_str, pct_str)
...
def plot_dae_results(cost_list, loss_list, batchcost, batchloss, sparsity_batch, beta_batch, sparsity_epoch, beta_epoch,
    ...
    if tied:
        # plot = plt.figure()
        # plt.title('HSP value')
        # for i in range(sparsity_batch.shape[1]):
        #     node_hsp_vec = sparsity_batch[:total_batches, i]
        #     plt.plot(node_hsp_vec)
        # plot.savefig('{}/hsp_batch.png'.format(output_folder))
        # plt.close()
        ...
        # for i in range(beta_batch.shape[1]):
        #     node_beta_vec = beta_batch[:total_batches, i]
        #     plt.plot(node_beta_vec)
        # plot.savefig('{}/beta_batch.png'.format(output_folder))
        # plt.close()
...
def train(model, optimizer, all_scans, beta_val, max_b, b_lr, tg_hsp, batchcost, batchloss, cost_list, loss_list,
    ...
    for block in range(num_blocks):
        ...
        for batch_idx in range(num_batches):
            ...
            # if tied:
            #     sparsity_batch[total_batches] = torch.clone(hsp_val[0]).detach().cpu().numpy()
            #     beta_batch[total_batches] = torch.clone(beta_val[0]).detach().cpu().numpy()
            # else:
            #     for i in range(2):
            #         sparsity_batch[i][total_batches] = hsp_val[i].data.cpu().numpy()
            #         beta_batch[i][total_batches] = beta_val[i].data.cpu().numpy()
            total_batches += 1
    if tied:
...
sparsity_batch = None
beta_batch = None
# sparsity_batch = np.zeros((n_batches, output_dim)) if tied else [np.zeros((n_batches, output_dim)),
#                                                                  np.zeros((n_batches, output_dim))]
# beta_batch = np.zeros((n_batches, output_dim)) if tied else [np.zeros((n_batches, output_dim)),
#                                                              np.zeros((n_batches, output_dim))]
