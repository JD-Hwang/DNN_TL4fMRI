...
print(hsp_str)
print(pct_str)
output_folder = '/users/nivl/data/autoencoder/classifier/{}/alltask/{}{}/{}{}{}_hsp{}_pct{}_{}_{}labels'.format(mode, month, day, hour, minute, sec, hsp_str, pct_str, pretrained_model, output_dim)
...
def validate(model, samples, labels, epoch):
    ...
    for batch_idx in range(len(samples)):
        data, target = torch.from_numpy(samples[batch_idx]), torch.from_numpy(labels[batch_idx])
        data, target = data.cuda(), target.cuda()
        with torch.no_grad():
            output = model(data)
            _, predicted = torch.max(output.data, 1)
...
# test_samples = stats.zscore(test_samples, axis=0)
...
if mode == 'finetune':
    if pretrained_model == 'ae':
        ...
        # net.encoder.weight.data = weight
        # net.encoder.bias.data = bias
...
for epoch in range(1, epochs + 1):
    ...
    if epoch % 10 == 0:
        plot_results(cost_list, loss_list, cost_batch, loss_batch, sparsity, betas, train_accuracies, test_accuracies, train_acc_batch, diff_dict, output_folder)
        torch.save(net.state_dict(), output_folder + '/model.pt')

mean_hsp = [torch.mean(hsp_val[j]).item() for j in range(len(hsp_val))]

sio.savemat(output_folder + '/val_acc.mat', {'accuracy': np.array(test_accuracies)})
sio.savemat(output_folder + '/val_acc_batch.mat', {'accuracy': np.array(val_acc_batch)})
pickle.dump(incorrect_dict, open('{}/incorrect_dict.p'.format(output_folder), 'wb'))
